...
AMY exposes exactly **one** portamento parameter — a time in ms (`portamento`, wire code `'m'`, C `uint16_t portamento_ms`). There is no glide curve, legato flag or portamento mode anywhere in its keyword table, so this single CC is the whole feature; a second control would have to be invented sketch-side, and polysynth has no Python in the note path to build it on (AMY routes notes natively via `grab_midi_notes=1`). Sent **per-oscillator** to the two sounding oscs, not per-synth: portamento's wire code is a bare `'m'` while every instrument-scoped keyword carries an `i` prefix (`synth_level` `'iV'`, `num_voices` `'iv'`), and the C declares `portamento_ms` inside `synthinfo`, AMY's per-oscillator struct. Requires the 2026-07-27 firmware or later.
