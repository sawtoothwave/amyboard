...
The MIDI CC map for Arctor, the AMYboard polysynth (`sketches/arctor.py`), received on **MIDI channel 12**. Each CC updates its parameter live — no voice reset, so held notes are never cut. Several CCs follow AMYboard/standard-MIDI defaults for backwards-compat — filter cutoff (74), resonance (71), the amp (VCA) ADSR (attack 73, decay 75, sustain 79, release 72), LFO rate (76) and vibrato depth (77); the rest use spare CCs.
...
| 34 | Porta Time | Portamento/glide time in ms, off a 25-rung **ladder** (`PORTA_MS_STEPS`): 5 ms steps to 50 ms, 50 ms to 500 ms, then 100 ms to 1 s. **0 = Off, the default** — AMY has no separate glide enable, so a zero time *is* the off switch. A spare CC deliberately, **not** MIDI's standard CC 5 "Portamento Time", for the same reason master volume avoids CC 7: a standard number risks firmware auto-mapping. See the Portamento note below. |
...
**Portamento (CC 34).** Glide between notes, on the **Osc** page under a `PORTA` header that **shares the `DRIFT` header row** — `DRIFT` centred over the two drift cells, `PORTA` over the two right-hand columns, with `TIME` centred beneath it. That sharing is the point: the Osc page was already *exactly* full (its content ends at y=123 against a 123px limit), so a `PORTA` section of its own would have paged the group. Mechanically it is one `section` run whose second half sets `hdr='Porta'` and `halfcol=True` — see `_Param.hdr` / `_grid_layout`. The split header supports **at most one row**, so the two halves can hold 2 + 2 params; a fifth wraps and they stop lining up.

AMY exposes exactly **one** portamento parameter — a time in ms (`portamento`, wire code `'m'`, C `uint16_t portamento_ms`). There is no glide curve, legato flag or portamento mode anywhere in its keyword table, so this single CC is the whole feature; a second control would have to be invented sketch-side, and Arctor has no Python in the note path to build it on (AMY routes notes natively via `grab_midi_notes=1`). Sent **per-oscillator** to the two sounding oscs, not per-synth: portamento's wire code is a bare `'m'` while every instrument-scoped keyword carries an `i` prefix (`synth_level` `'iV'`, `num_voices` `'iv'`), and the C declares `portamento_ms` inside `synthinfo`, AMY's per-oscillator struct. Requires the 2026-07-27 firmware or later.
