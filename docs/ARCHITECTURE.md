...
- Explicit Python-defined synth graph for osc 0-3 instead of relying on web-editor autogenerated patch state
...
**AMY Synthesizer**
- Silent control oscillator on osc 0 handling note gate, VCA ADSR, and VCF for the voice chain
- Dual tone oscillators (A/B on osc 2 and 3) with independent waveforms, pitch, duty cycle
- Multi-mode filter (LP/BP/HP) with ADSR modulation on the control oscillator
- Dual ADSR envelopes (VCF + VCA) applied from the control oscillator across the chained tone sources
