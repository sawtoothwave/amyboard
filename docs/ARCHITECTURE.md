...
- MIDI channel-12 notes are auto-routed to synth 12 by AMY (synth number N == MIDI channel N); CCs are handled via `midi.add_callback(midi_cb)`. CV1 provides 1V/oct monophonic pitch and CV2 a gate, polled from `loop()`.
- To stay channel-12-only, `init_synth()` zeroes the voice count of every synth except 12, removing the default instrument the firmware allocates on synth 1 (channel 1); a voiceless synth cannot sound.
